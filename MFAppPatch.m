...
static BOOL apSwiftTextPatchDump(NSDictionary *d, NSData *oldBytes, NSData *newBytes, NSString **err) {
    ...
    //    存的 slide 快照只在当次会话有效, 冷启动必过期(dbg_31 实锤:
    ...
    // v2.58.123: 双守卫组 — 真门带 guard2(cbz 提前判空)时一并 patch。
    //   target-app 形态: ldrb tag → cbz(跳走) → cmp#1 → b.ne; 只 NOP b.ne 时
    //   tag==0 仍被 cbz 拦住 → 必须把 cbz 也 NOP 掉才能进有值路径。
    //   注: guard2 失败不改变主门结果(单独记日志)。
    if (ok && [d[@"guard2"] isKindOfClass:[NSDictionary class]]) {
        NSDictionary *g2 = d[@"guard2"];
        if (g2[@"vmaddr"]) {
            uintptr_t g2abs = (uintptr_t)[g2[@"vmaddr"] unsignedLongLongValue] + (uintptr_t)slide;
            NSData *g2old = ([g2[@"old"] isKindOfClass:[NSString class]] && [g2[@"old"] length]) ? apHexToBytes(g2[@"old"]) : nil;
            NSData *g2new = ([g2[@"new"] isKindOfClass:[NSString class]] && [g2[@"new"] length]) ? apHexToBytes(g2[@"new"]) : apHexToBytes(@"1f2003d5");
            NSString *g2err = nil;
            if (apTextPatchAt(g2abs, g2old, g2new, &g2err))
                apLog(@"[swifttext] %@ guard2(cbz) abs=%#lx → patch OK (双守卫组)", imgName, (unsigned long)g2abs);
            else
                apLog(@"[swifttext] %@ guard2(cbz) abs=%#lx patch 失败: %@", imgName, (unsigned long)g2abs, g2err ?: @"?");
        }
    }
    return ok;
}
...
static void apEntDumpsLoad(void) {
    ...
    dispatch_once(&once, ^{
        ...
        //   启动即剔除, 不进重打清单。dbg_72: deepslot@11d1398.9 僵尸在库 on=YES
...
//   → dbg_75 用户看到 35 点(侦查详情只报 12, 库里是历史累积)。
...
//   dbg_72 定谳: F10 每次侦查都扫出同族点(11d1398=CustomerInfo 序列化族,
...
void apEntDumpsApply(void) {
    ...
    for (NSDictionary *d in g_entDumps) {
        ...
        if ([d[@"sym"] hasPrefix:@"ivarRead@"]) {
            ...
            //   (写侧 refresh 未购买态可能不跑, dbg_65 sk2diag=nil 实锤)
            if ([d[@"new"] isKindOfClass:[NSString class]] && [d[@"new"] length])
                newBytes = apHexToBytes(d[@"new"]);
        ...
        } else if ([d[@"sym"] hasPrefix:@"manual@"]) {
            // v2.58.139: 手动录入点 — 逆向定位的精确 patch, 严格用库内 new 字节(4字节指令替换)。
            //   无兜底: manual 点必须自带 old/new(录入时已确定), 缺失即跳过, 绝不瞎打。
            if ([d[@"new"] isKindOfClass:[NSString class]] && [d[@"new"] length])
                newBytes = apHexToBytes(d[@"new"]);
        ...
        //   结果: 既不解锁(返回值被覆盖), 又毁栈(dbg_85 四序言点即此形态, 全部空转)。
...
// v2.58.139: 手动 patch 点录入 — 逆向定位的成果(vmaddr + old/new 字节)录进持久层。
//   意义: 引擎的自动扫描(F8门形态/F9存储)对"服务器票据型内存字典门"等复杂形态命中不了,
//   逆向人工定位后需要一条通用录入通道 —— 不硬编码进代码(改一版死一版),
//   而是像侦查结果一样进 mfEntDumps 持久化, 冷启动 apEntDumpsApply 自动重打。
//   bundleID 门控: 只对匹配的 app 录入(录进当前进程的持久层, 天然按 app 隔离)。
//   sym 用 manual@<vmaddr> 唯一标识, 可左划删除/开关/回滚, 与侦查点位同等对待。
// 参数: vmaddr=镜像内静态 VA(含 __TEXT base, 如 0x1027c75bc); oldHex/newHex=小端字节 hex
BOOL mfAppPatchEntManualAdd(NSString *bundleMatch, unsigned long long vmaddr,
                            NSString *oldHex, NSString *newHex, NSString *note) {
    NSString *bid = [[NSBundle mainBundle] bundleIdentifier] ?: @"";
    if (bundleMatch.length && ![bid containsString:bundleMatch]) return NO;   // 非目标 app 不录
    NSString *mainPath = [[NSBundle mainBundle] executablePath];
    NSString *img = mainPath ? [mainPath lastPathComponent] : @"main";
    NSString *sym = [NSString stringWithFormat:@"manual@%#llx", vmaddr];
    NSDictionary *dump = @{
        @"img": img,
        @"sym": sym,
        @"vmaddr": @(vmaddr),
    ...
    mfAppPatchEntDumpsMerge(@[dump]);
    apLog(@"[entdump] ✍️ 手动录入 %@ vmaddr=%#llx %@→%@ (%@)", sym, vmaddr, oldHex, newHex, note ?: @"");
    return YES;
}
...
// v2.58.119: 批量路径用的存储操作(定义在文件前部, 显式 extern 明确意图)
extern void mfAppPatchEntDumpSetOn(NSString *sym, BOOL on);
extern void mfAppPatchEntDumpDelete(NSString *sym);
// v2.58.119: 批量操作共用 — 计算点位 patch 字节(单一实现, 与 ⚡/持久化路径同语义)
static NSData *apEntNewBytesFor(NSDictionary *d, NSString **err) {
    NSString *sym = d[@"sym"] ?: @"";
    if ([sym hasPrefix:@"ivarRead@"]) {
        ...
        uint32_t movz = 0x52800020u | rt;   // mov w<rt>,#1
        return [NSData dataWithBytes:&movz length:4];
    }
    if ([sym hasPrefix:@"deepslot@"]) {
        unsigned rt = 9;
        NSRange dot = [sym rangeOfString:@"." options:NSBackwardsSearch];
        if (dot.location != NSNotFound) rt = (unsigned)[[sym substringFromIndex:dot.location + 1] intValue];
        if (rt > 30) rt = 9;
...
// v2.58.119: 批量操作共用 — 解析点位绝对地址(镜像查找 + 当前 slide)
static uintptr_t apEntAbsAddr(NSDictionary *d) {
    NSString *imgName = d[@"img"] ?: @"";
    if (!imgName.length || !d[@"vmaddr"]) return 0;
    const char *want = imgName.UTF8String;
    uint32_t ic = _dyld_image_count();
    for (uint32_t i = 0; i < ic; i++) {
        const char *n = _dyld_get_image_name(i);
        if (n && strstr(n, want))
            return (uintptr_t)[d[@"vmaddr"] unsignedLongLongValue] + (uintptr_t)_dyld_get_image_vmaddr_slide(i);
    }
    return 0;
}
// v2.58.119: 一键 patch 全部 — 遍历当前库全部点位(跳过 ptr/序言), ⚡成功即持久化
- (void)mfAPBatchPatchAll:(UIButton *)btn {
    NSArray *all = [mfAppPatchEntDumps() copy];   // 快照 — 循环内 SetOn 改字典内容, 不能直接枚举库
    if (!all.count) { mfToast(@"无点位可 patch"); return; }
    if (![objc_getAssociatedObject(btn, "mfArmBatch") boolValue]) {
        objc_setAssociatedObject(btn, "mfArmBatch", @YES, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        [btn setTitle:[NSString stringWithFormat:@"⚠️ 再点确认 patch %lu 点", (unsigned long)all.count]
             forState:UIControlStateNormal];
        btn.backgroundColor = [UIColor systemRedColor];
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(3 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            if ([objc_getAssociatedObject(btn, "mfArmBatch") boolValue]) {
                objc_setAssociatedObject(btn, "mfArmBatch", @NO, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
                [btn setTitle:[NSString stringWithFormat:@"⚡ 全部 patch(%lu)", (unsigned long)mfAppPatchEntDumps().count]
                     forState:UIControlStateNormal];
    ...
    objc_setAssociatedObject(btn, "mfArmBatch", @NO, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    NSUInteger ok = 0, fail = 0, skip = 0, already = 0;
    for (NSDictionary *d in all) {
        ...
        if (ob.length >= 4) {
            uint32_t o0 = 0; [ob getBytes:&o0 length:4];
            BOOL isPrologue = ((o0 & 0xFFC003FF) == 0xD10003FF && ((o0 >> 10) & 0xFFF)) ||
                              ((o0 & 0x7FC00000) == 0x29800000 && ((o0 >> 5) & 0x1F) == 31) ||
                              (o0 == 0xD503237F);
        ...
        uintptr_t abs = apEntAbsAddr(d);
        BOOL g2Pending = NO;
        if ([d[@"guard2"] isKindOfClass:[NSDictionary class]] && d[@"guard2"][@"vmaddr"]) {
            uintptr_t g2abs = (uintptr_t)[d[@"guard2"][@"vmaddr"] unsignedLongLongValue] + (uintptr_t)_dyld_get_image_vmaddr_slide(0);
            // 用镜像 slide 更准: 复用 apEntAbsAddr 同口径
            NSDictionary *g2d = @{ @"img": d[@"img"] ?: @"", @"vmaddr": d[@"guard2"][@"vmaddr"] };
            g2abs = apEntAbsAddr(g2d);
            NSData *g2old = ([d[@"guard2"][@"old"] isKindOfClass:[NSString class]] && [d[@"guard2"][@"old"] length]) ? apHexToBytes(d[@"guard2"][@"old"]) : nil;
            if (g2abs && g2old.length >= 4) {
                NSData *cur2 = [NSData dataWithBytes:(void*)g2abs length:4];
                NSData *ob2 = [g2old subdataWithRange:NSMakeRange(0, 4)];
                if ([cur2 isEqualToData:ob2]) g2Pending = YES;   // guard2 还是原字节 → 需要打
            }
        }
        if (abs && nb.length >= 4 && !g2Pending) {
            uint32_t cur0 = *(const uint32_t *)abs, want0 = 0;
            [nb getBytes:&want0 length:4];
            if (cur0 == want0) { already++; mfAppPatchEntDumpSetOn(sym, YES); continue; }
        }
        NSString *err = nil;
        if (apSwiftTextPatchDump(d, ob, nb, &err)) {
            g_apHits++;
            mfAppPatchEntDumpSetOn(sym, YES);   // ⚡即持久化(与单点同语义)
            ok++;
        } else {
            fail++;
            apLog(@"[entdump] 批量 patch 失败 %@: %@", sym, err ?: @"?");
        }
    }
    apLog(@"[entdump] ⚡批量 patch: ok=%lu 已打=%lu fail=%lu skip=%lu (共 %lu)",
          (unsigned long)ok, (unsigned long)already, (unsigned long)fail, (unsigned long)skip, (unsigned long)all.count);
    mfToast([NSString stringWithFormat:@"⚡ 成功 %lu · 已打 %lu · 失败 %lu · 跳过 %lu",
             (unsigned long)ok, (unsigned long)already, (unsigned long)fail, (unsigned long)skip]);
    for (UIButton *b in btn.superview.subviews)
        if ([b isKindOfClass:[UIButton class]] && b.tag == 301)
            [b setTitle:[NSString stringWithFormat:@"⚡ 全部 patch(%lu)", (unsigned long)mfAppPatchEntDumps().count]
                forState:UIControlStateNormal];
    UITableView *tv = objc_getAssociatedObject(self, "mfAPEntTV");
    [tv reloadData];
}
// v2.58.119: 一键回滚并删除释放全部 — 先还原字节(old 写回), 再逐点删除+立墓碑
- (void)mfAPBatchRevertAll:(UIButton *)btn {
    NSArray *all = [mfAppPatchEntDumps() copy];   // 快照 — 循环内 Delete 改库, 不能直接枚举
    if (!all.count) { mfToast(@"无点位"); return; }
    if (![objc_getAssociatedObject(btn, "mfArmRevert") boolValue]) {
        objc_setAssociatedObject(btn, "mfArmRevert", @YES, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
        [btn setTitle:[NSString stringWithFormat:@"⚠️ 再点确认回滚+删除 %lu 点", (unsigned long)all.count]
             forState:UIControlStateNormal];
        btn.backgroundColor = [UIColor systemRedColor];
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(3 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            if ([objc_getAssociatedObject(btn, "mfArmRevert") boolValue]) {
                objc_setAssociatedObject(btn, "mfArmRevert", @NO, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
                [btn setTitle:@"♻️ 全部回滚并删除" forState:UIControlStateNormal];
                btn.backgroundColor = [UIColor systemRedColor];
            }
        });
        return;
    ...
    objc_setAssociatedObject(btn, "mfArmRevert", @NO, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    NSUInteger rev = 0, clean = 0, del = 0;
    for (NSDictionary *d in all) {
        NSString *sym = d[@"sym"] ?: @"";
        NSString *old = d[@"old"];
        // 回滚: 当前字节 ≠ old(已 patch) → 写回 old; 相等 = 本来干净
        if ([old isKindOfClass:[NSString class]] && old.length >= 8) {
            NSData *ob = apHexToBytes(old);
            uintptr_t abs = apEntAbsAddr(d);
            if (abs && ob.length >= 4) {
                NSData *cur = [NSData dataWithBytes:(void*)abs length:4];
                NSData *ob4 = [ob subdataWithRange:NSMakeRange(0, 4)];   // 只回写 4 字节(patch 粒度)
                if (![cur isEqualToData:ob4]) {
                    NSString *rerr = nil;
                    if (apTextPatchAt(abs, nil, ob4, &rerr)) rev++;
                    else apLog(@"[entdump] 批量回滚失败 %@: %@", sym, rerr ?: @"?");
                } else clean++;
            }
        }
        // 删除 + 立墓碑(与单点 ✂ 同语义: 侦查不再复活)
        if (sym.length) { mfAppPatchEntDumpDelete(sym); del++; }
    }
    apLog(@"[entdump] ♻️ 批量回滚+删除: revert=%lu 本净=%lu delete=%lu", (unsigned long)rev, (unsigned long)clean, (unsigned long)del);
    mfToast([NSString stringWithFormat:@"♻️ 已回滚 %lu · 已删 %lu 点", (unsigned long)rev, (unsigned long)del]);
    UITableView *tv = objc_getAssociatedObject(self, "mfAPEntTV");
    g_apEntList.items = @[];
    [tv reloadData];
    for (UIButton *b in btn.superview.subviews) {
        if (![b isKindOfClass:[UIButton class]]) continue;
        if (b.tag == 301) [b setTitle:@"⚡ 全部 patch(0)" forState:UIControlStateNormal];
        if (b.tag == 302) [b setTitle:@"♻️ 已清空 — 可点♻️恢复" forState:UIControlStateNormal];
    }
}
...
- (void)mfAPShowEntDumps {
    ...
    // v2.58.27: 按 score 降序显示 — 旧序=插入序(F8v2 旧候选堆在前), dbg_26 实锤
    ...
    // v2.58.119: 一键批量条 — 用户要求"一键patch全部 / 一键回滚并删除释放全部判定点"
    CGFloat batchH = 0;
    {
        ...
        b1.tag = 301;
        b1.titleLabel.font = [UIFont systemFontOfSize:12.5 weight:UIFontWeightMedium];
        [b1 setTitle:[NSString stringWithFormat:@"⚡ 全部 patch(%lu)", (unsigned long)sorted.count]
            forState:UIControlStateNormal];
        ...
        b2.tag = 302;
        b2.titleLabel.font = [UIFont systemFontOfSize:12.5 weight:UIFontWeightMedium];
        [b2 setTitle:@"♻️ 全部回滚并删除" forState:UIControlStateNormal];
    ...
    [page addSubview:tv];
    objc_setAssociatedObject(self, "mfAPEntTV", tv, OBJC_ASSOCIATION_RETAIN_NONATOMIC);   // v2.58.119: 批量操作后刷表
    mfPushPage(page);
}
...
- (void)mfAPEntPatchNow:(NSString *)sym {
    ...
    // v2.58.28: ivarRead@ 前缀 = ldrb 指令点(dbg_27 实锤: @Observable getter
    ...
    for (NSDictionary *d in mfAppPatchEntDumps()) {
        ...
        } else if ([sym hasPrefix:@"manual@"]) {
            // v2.58.141 (dbg_132 定谳): 单点 ⚡ 缺 manual 分支 → 落 else 打 8 字节
            //   mov w0,#1;ret, 覆盖 mov w22,#0 后两条指令且 old 校验(4字节)对不上 → 打不上。
            //   manual 点严格用库内 new(录入时确定的精确字节), 缺失即拒绝, 绝不瞎打。
            if ([d[@"new"] isKindOfClass:[NSString class]] && [d[@"new"] length])
                newBytes = apHexToBytes(d[@"new"]);
        ...
        NSData *oldBytes = ([d[@"old"] isKindOfClass:[NSString class]] && [d[@"old"] length]) ? apHexToBytes(d[@"old"]) : nil;
        // v2.58.85 (dbg_87 定谳): 序言形态拦截必须在 ⚡ 立即路径也有 —
...
void mfAppPatchSectionInLabPage(UIView *page, CGFloat *yio) {
    ...
    //   显示类型构成; 旧的 🧩 第二张同义卡已删(侦查→入库→一张卡看全)。
    {
        ...
        //   用户看不出哪些是真解锁候选(dbg_79: "12 个"里 8 个是共享 bool 噪声)。
    ...
    // v2.58.132 加的「权益数据直写」按钮已删(2.58.133): 重复造轮子 — F9 本就是直写工具,
    //   且已扩成标准+App Group 双存储, 能定位真实权益 key。数据侧直写走 F9, 不再单开按钮。
