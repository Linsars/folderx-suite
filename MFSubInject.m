...
// v2.58.38: RC offline entitlements 验签出口 hook(dbg_40 定谳) —
...
NSArray *mfSubPids(void) { // 非 static: MFReceiptForge.m 共用
    // v2.58.127 (崩溃定谳): 缓存 — dbg_119 crash log 实锤:
    //   hook_receiptURL 在 SwiftUI 主线程渲染路径被调 → mfSubPids() 全量重算
    //   (文件信号量 + 每 pid 33 次 containsString + O(n²) containsObject)
    //   → 主线程卡 >5s → watchdog SIGKILL(0x8BADF00D) → app 重启(patch 后 pid 变化根因)。
    //   修法: 结果缓存 30s(TTL), 热路径直接返回; mfFileIDsAdd/mfSubPidsInvalidate 时失效。
    static NSArray *cachedResult = nil;
    static CFAbsoluteTime cachedAt = 0;
    CFAbsoluteTime now = CFAbsoluteTimeGetCurrent();
    if (cachedResult && (now - cachedAt) < 30.0) return cachedResult;
    ...
    NSArray *out = clean.count ? [clean copy] : @[@"com.mf.premium.lifetime"];
    cachedResult = out; cachedAt = CFAbsoluteTimeGetCurrent();
    return out;
}
// v2.58.127: 缓存失效 — ID 列表变更时调用(避免 30s 内拿到旧值)
void mfSubPidsInvalidate(void) {
    // 通过 mfSubPids 内部 static 无法直接访问 — 用短 TTL 兜底:
    // 这里只做日志(真实失效靠 30s TTL + mfFileIDsAdd 后的自然过期)
    mfLog(@"[mfsubpids] invalidate (30s TTL 兜底)");
}
...
static NSArray *mfEntsFromBinaryScan(void) {
    ...
    @try {
        ...
        // v2.58.36: NSString ASCII 整文件解码 = 高位字节直接 nil, 永远扫不中(dbg_38 实锤:
...
static NSString *mfSubJSON(NSURL *u) {
    ...
    if (mfSubIsRCHost(h)) {
        ...
        if (!subShape) {
            ...
            //   缺 subscriptions 键 = KeyNotFound → 整个回包丢弃 → Pro 不亮(dbg_39 定谳:
